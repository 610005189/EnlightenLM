...
基于 HuggingFace Transformers 的 LLaMA 模型支持。
"""
...
@ModelFactory.register(ModelProvider.LLAMA)
class LLaMAModel(BaseModelInterface):
    """LLaMA 模型实现

    基于 HuggingFace Transformers 实现，支持本地 LLaMA 模型推理。
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._max_seq_len = self.config.get("max_seq_len", 2048)
        ...
        try:
            ...
            if self._load_in_8bit or self._load_in_4bit:
                ...
                self._model = AutoModelForCausalLM.from_pretrained(
                    ...
                    revision=self._revision,
                    torch_dtype=self._get_dtype(),
                    attn_implementation="flash_attention_2" if self._use_flash_attention else "eager"
                )
            ...
            raise ModelLoadError(f"LLaMA load failed: {e}") from e
    ...
    def get_capabilities(self) -> ModelCapability:
        ...
        return ModelCapability(
            ...
            max_context_length=self._metadata.context_length if self._metadata else self._max_seq_len,
    ...
    def chat(
        ...
        **kwargs
        ...
        prompt = self._format_llama_chat_prompt(messages)
    ...
    def _format_llama_chat_prompt(self, messages: List[Dict[str, str]]) -> str:
        """格式化聊天消息为 LLaMA 格式的 prompt"""
        if not messages:
            return ""
