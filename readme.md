...
[![Tests](https://img.shields.io/badge/tests-23%20passed-green)]()
...
当前版本已实现完整的 L3 安全监控体系（敏感词检测、自指循环检测、词汇重复检测、智能冷却机制、元认知自检）、L2 会话历史与统计特征提取、贝叶斯病因推断器（区分"先验偏见"与"感知噪声"）、Engram锚点和rewind机制（记忆状态保存和回退）、Merkle树审计验证、密码学审计链。项目支持 Ollama、DeepSeek API 等多种模型后端，提供 REST API 和 Web 界面，已通过 23+ 个单元测试。
...
运行核心测试（当前 23+ 个用例全部通过）：

```bash
pytest tests/test_merkle_tree.py tests/test_metacognition.py tests/test_engram.py -v
```
...
1. **架构定位收拢与文档更新**
2. **审计链激活** - 将已实现的哈希链与 HMAC 签名模块接入实际推理管线
3. **Logits 级实时熵监控** - 收集标注数据，离线训练轻量级 MLP 幻觉判别器

### 中期（1~3 个月）

4. **偏见检测与缓解（P0）** - 集成多领域敏感词库，触发后追加元认知自检提示
5. **偏见检测与缓解（P1）** - 构建有偏/无偏语句对比数据集，训练偏见探针
6. **干预策略精细化** - 将现有的 halt 升级为三级策略：continue / halt / rewind
...
**维护者**：EnlightenLM 团队
**最后更新**：2026-04-26
**版本**：v3.1 - 三层安全监控 + 贝叶斯推断 + 元认知自检 + Engram锚点 + Merkle树审计 + 实时熵监控 + WebSocket 实时监控 + Prometheus/Grafana 监控已就绪